...
export default function TradeRegister() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Basic validation
    if (!form.tradeName || !form.tradeType || !form.licenseNumber || !form.address || !form.email || !form.phone || !form.password || !form.confirm) {
    ...
    if (licenseStatus !== "valid") {
      setError("Please validate your Trade License Number before continuing.");
      return;
    }
  ...
  return (
    <div style={{ maxWidth: 480, margin: "40px auto", background: "#fff", padding: 32, borderRadius: 16, boxShadow: "0 2px 8px #0001" }}>
      {step === "form" ? (
        <>
          <h1 style={{ fontSize: 28, fontWeight: "bold", marginBottom: 8 }}>Register as a Qualified Tradesperson</h1>
          ...
          <form onSubmit={handleSubmit} autoComplete="off">
            <div style={{ marginBottom: 14 }}>
              ...
              <input name="tradeName" value={form.tradeName} onChange={handleChange} className="form-input" required style={{ width: "100%", padding: 8, borderRadius: 6, border: "1px solid #ccc" }} />
            </div>
            <div style={{ marginBottom: 14 }}>
              <label>Trade Type</label>
              <input name="tradeType" value={form.tradeType} onChange={handleChange} className="form-input" required placeholder="e.g. Electrician, Plumber" style={{ width: "100%", padding: 8, borderRadius: 6, border: "1px solid #ccc" }} />
            </div>
            <div style={{ marginBottom: 14 }}>
        ...
        <div>
          ...
          <p>Your trade profile will be activated after you choose a subscription plan and your license is verified.</p>
        </div>
      )}
    </div>
  );
